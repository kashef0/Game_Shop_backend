  ...
  try {
    ...
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({
          message: "Inte auktoriserad att lägga till eller uppdatera spel",
        });
    }

    // Extrahera de olika fälten från request kroppen
    const { rawgId, price, rentalPrice, stock, availableForRent, isActive } =
      req.body;
...
exports.updateGame = async (req, res) => {
  try {
    ...
    const { price, rentalPrice, stock, availableForRent, isActive } = req.body;
